...
export async function getSingleProductSupplier(
    ...
    try {
        ...
        const findSingleProductSupplier = await ProductSupplier.findOne({ _id: ProductSupplierId });
...
export async function createProductSupplier(
    ...
    try {
        const ProductSupplierId = req.params.id;
        const checkExisting = await ProductSupplier.findOne({ _id: ProductSupplierId });
        if (checkExisting) {
            return res.status(400).json({ message: "ProductSupplier already exists" });
        }
        ...
        return res
            .status(201)
            .json({ message: "ProductSupplier created successfully", data: newProductSupplier });
    } catch (error) {
...
export async function deleteProductSupplier(
    ...
    const productSupplierId = req.params.id;
    try {
        const deletedProductSupplier = await ProductSupplier.findOneAndDelete({ _id: productSupplierId });

        if (!deletedProductSupplier) {
            return res.status(404).json({ message: "ProductSupplier not found" });
        }

        return res.status(200).json({ message: "ProductSupplier deleted successfully" });
    } catch (error) {
